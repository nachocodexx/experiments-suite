...
//    val WRITES_JSONs: List[Path] = WRITE_BASE_FOLDER_PATH.toFile.listFiles().toList.map(_.toPath)
    val clientResource: Resource[IO, Client[IO]] = BlazeClientBuilder[IO](global)
      .withRequestTimeout(config.requestTimeoutMs milliseconds)
      .withConnectTimeout(config.connectTimeoutMs milliseconds)
      .withMaxWaitQueueLimit(config.maxWaitQueueLimit)
      .withMaxTotalConnections(config.maxTotalConnections)
      .withIdleTimeout(config.idleTimeout milliseconds)
